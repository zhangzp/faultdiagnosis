...
%             load(['D:\ZhangProject\mat\matf',num2str(faulti),'\teline',num2str(timen),'-',num2str(tagn),'-',num2str(faulti),'.mat']);
            ...
            while (mini>(size(classerr,1)-11) || (classerr(end,1)>(classerr(end,2)-0.00005) && mini1>(size(classerr,1)-11))) && miserri>(size(classerr,1)-201) && size(classerr,1)<1800 % && miniv2>0.00001
                [ classifier,claerr] = classifytrain(1,classifier,traindata,trainlabel,testdata,testlabel,faulti,nori,1,0);
